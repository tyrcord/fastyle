...
// It is designed to be used as a child of the CalculatorKeyboard widget to
// allow the calculator to receive keyboard events when it has focus.
...
class FastDigitCalculatorKeyboardListenerState
  ...
  // This function determines which action to take based on the logical
  // key received
  ...
  KeyEventResult _handleLogicalKey(LogicalKeyboardKey logicalKey) {
    ...
    // If the logical key represents a digit, extract the corresponding
    // character from its label
    if (_isDigitKey(logicalKey)) {
      final digit = _getDigitFromKeyLabel(logicalKey.keyLabel);
      // Call the onKeyPressed callback provided by the parent widget with
      // the digit
  ...
  // This function takes a key label and returns the digit contained in
  // it (if any)
